...
const db = {
  ...
  deletePlan: (id) => sb(`meal_plans?id=eq.${id}`, { method: "DELETE", prefer: "return=minimal" }),
...
const btnPrimary = { background: `linear-gradient(135deg,${G},#a8852e)`, border: "none", color: "#060606", padding: "11px 20px", fontSize: 11, letterSpacing: 3, borderRadius: 3, cursor: "pointer", fontWeight: "bold", fontFamily: "inherit" };
...
function Login({ onLogin }) {
  ...
  function submit() { if (pw === COACH_PASSWORD) onLogin(); else setErr("Contrasena incorrecta"); }
  return (
    <div style={{ minHeight: "100vh", background: BG, display: "flex", alignItems: "center", justifyContent: "center", padding: 24, fontFamily: "Palatino Linotype, serif" }}>
      <div style={{ background: "#0c0c0c", border: `1px solid ${GB}`, borderRadius: 4, padding: "44px 32px", maxWidth: 380, width: "100%", textAlign: "center" }}>
        ...
        <div style={{ color: G, fontSize: 9, letterSpacing: 5, textTransform: "uppercase", marginBottom: 4 }}>Jose Leiro</div>
        <div style={{ color: "#fff", fontSize: 19, letterSpacing: 2, marginBottom: 4 }}>Panel de Coach</div>
        <GoldLine />
        <input type="password" placeholder="Contrasena del coach" value={pw} onChange={e => setPw(e.target.value)} onKeyDown={e => e.key === "Enter" && submit()} style={{ ...iS, marginBottom: 12, textAlign: "center" }} />
  ...
  return (
    <div style={{ padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          <div style={{ color: G, fontSize: 9, letterSpacing: 4, textTransform: "uppercase" }}>Jose Leiro Elite Coach</div>
      ...
      {loading ? <div style={{ textAlign: "center", color: "rgba(255,255,255,0.3)", padding: 40 }}>Cargando...</div> :
        clients.length === 0 ? <div style={{ textAlign: "center", color: "rgba(255,255,255,0.2)", padding: 40, lineHeight: 2 }}>Sin clientes aun.</div> :
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {clients.map(c => (
              <div key={c.id} onClick={() => onSelect(c)} style={{ background: SF, border: `1px solid ${GB}`, borderRadius: 4, padding: "14px 16px", cursor: "pointer", display: "flex", alignItems: "center", gap: 14 }}>
                <div style={{ width: 42, height: 42, borderRadius: "50%", background: GD, border: `1px solid ${GB}`, display: "flex", alignItems: "center", justifyContent: "center", color: G, fontSize: 14, fontWeight: "bold", flexShrink: 0 }}>{c.name?.charAt(0).toUpperCase()}</div>
...
function NewClientForm({ onSave, onCancel }) {
  const [form, setForm] = useState({ name: "", email: "", phone: "", gender: "", birth_date: "", country: "Espana", city: "", weight_kg: "", height_cm: "", goal: "", notes: "" });
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <Card>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[["name","Nombre completo *","text"],["email","Email *","email"],["phone","Telefono","text"],["birth_date","Fecha nacimiento","date"],["country","Pais","text"],["city","Ciudad","text"]].map(([k,l,t]) => (
        ...
        <Card>
          ...
          <div><Label>Objetivo</Label><input value={form.goal} onChange={e => set("goal", e.target.value)} style={{ ...iS, marginBottom: 10 }} /></div>
    ...
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
      <div style={{ padding: "14px 16px", borderBottom: `1px solid ${GB}`, display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 10, marginTop: 1 }}>{client.goal || "Sin objetivo"}</div>
        </div>
      </div>
...
function PlansTab({ client }) {
  ...
  async function generatePlan() {
    ...
    try {
      ...
      const plan = await generateMealPlan(client, numMeals, prompt, weeks);
      const calMatch = prompt.match(/\b(\d{3,4})\s*kcal/i);
      const totalCals = calMatch ? parseInt(calMatch[1]) : plan.total_calories;
      const title = planName.trim() || plan.plan_title;
      ...
      for (const week of plan.weeks) {
        for (const day of week.days) {
          ...
          for (const meal of day.meals) {
            const savedMeal = await db.createMeal({ plan_day_id: savedDay.id, meal_order: meal.meal_order, name: meal.name, time_of_day: meal.time_of_day, description: meal.description, image_url: getFoodImage(meal.name), calories: meal.calories, protein_g: meal.protein_g, carbs_g: meal.carbs_g, fat_g: meal.fat_g, recipe: meal.recipe });
      ...
      setPlans(await db.getPlans(client.id));
      setCreating(false);
      setPlanName(""); setPrompt("");
  ...
  async function handleDelete(planId) {
    if (!confirm("Borrar este plan? Esta accion no se puede deshacer.")) return;
    ...
    <div style={{ padding: 14 }}>
      {!creating ? (
        ...
        <Card style={{ marginBottom: 16 }}>
          ...
          <input value={planName} onChange={e => setPlanName(e.target.value)} placeholder="Ej: Plan verano Ana - 1850 kcal" style={{ ...iS, marginBottom: 10 }} />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            <div><Label>Comidas por dia</Label>
              <select value={numMeals} onChange={e => setNumMeals(+e.target.value)} style={iS}>
                {[3,4,5,6].map(n => <option key={n} value={n}>{n} comidas</option>)}
              </select>
            </div>
            <div><Label>Duracion</Label>
              <select value={weeks} onChange={e => setWeeks(+e.target.value)} style={iS}>
          ...
          <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="Ej: Plan de 1850 kcal para Ana, dieta veraniega con alimentos espanoles..." rows={4} style={{ ...iS, resize: "vertical", marginBottom: 12 }} />
          <div style={{ display: "flex", gap: 10 }}>
            <button onClick={generatePlan} disabled={genLoading || !prompt.trim()} style={{ ...btnPrimary, flex: 1 }}>{genLoading ? "Generando..." : "Generar Plan"}</button>
            <button onClick={() => setCreating(false)} style={btnGhost}>Cancelar</button>
          </div>
          {genLoading && <div style={{ textAlign: "center", color: "rgba(255,255,255,0.3)", fontSize: 11, marginTop: 10, letterSpacing: 2 }}>CREANDO PLAN... (puede tardar 30-60 seg)</div>}
        </Card>
      )}
      {loading ? <div style={{ textAlign: "center", color: "rgba(255,255,255,0.3)", padding: 30 }}>Cargando...</div> :
        plans.length === 0 ? <div style={{ textAlign: "center", color: "rgba(255,255,255,0.2)", padding: 30 }}>Sin planes aun.</div> :
          plans.map(p => (
            <Card key={p.id} style={{ marginBottom: 10 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                <button onClick={() => setSelectedPlan(p)} style={{ ...btnGhost, flex: 1, fontSize: 10 }}>Editar</button>
              ...
              <button onClick={() => handleDelete(p.id)} style={{ ...btnGhost, width: "100%", marginTop: 6, fontSize: 10, color: "#ff6b6b", borderColor: "rgba(255,107,107,0.3)" }}>Borrar Plan</button>
            </Card>
          ))
      }
    </div>
...
function PlanEditor({ plan, onBack }) {
  ...
  return (
    <div style={{ padding: 14 }}>
      ...
      {loading ? <div style={{ textAlign: "center", color: "rgba(255,255,255,0.3)", padding: 30 }}>Cargando dias...</div> : (
        <>
        ...
        meals.map(m => (
          <Card key={m.id} style={{ marginBottom: 10 }}>
            <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
              ...
              <button onClick={() => setEditMeal(m)} style={{ ...btnGhost, padding: "6px 10px", fontSize: 12 }}>Editar</button>
            </div>
          </Card>
        ))
      }
...
function MealEditor({ meal, onSave, onCancel }) {
  ...
  return (
    <div style={{ padding: "0 0 16px" }}>
      ...
      <Card>
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[["name","Nombre"],["time_of_day","Hora"],["description","Descripcion"]].map(([k,l]) => (
            <div key={k}><Label>{l}</Label><input value={form[k] || ""} onChange={e => set(k, e.target.value)} style={iS} /></div>
          ))}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[["calories","Calorias"],["protein_g","Proteina (g)"],["carbs_g","Carbos (g)"],["fat_g","Grasas (g)"]].map(([k,l]) => (
              <div key={k}><Label>{l}</Label><input type="number" value={form[k] || ""} onChange={e => set(k, e.target.value)} style={iS} /></div>
            ))}
          </div>
          <div><Label>Receta</Label><textarea value={form.recipe || ""} onChange={e => set("recipe", e.target.value)} rows={5} style={{ ...iS, resize: "vertical" }} /></div>
          <div><Label>URL imagen (opcional)</Label><input value={form.image_url || ""} onChange={e => set("image_url", e.target.value)} style={iS} /></div>
        </div>
      </Card>
...
function ChecksTab({ client }) {
  ...
  return (
    <div style={{ padding: 14 }}>
      ...
      {loading ? <div style={{ textAlign: "center", color: "rgba(255,255,255,0.3)", padding: 30 }}>Cargando...</div> :
        checks.length === 0 ? <div style={{ textAlign: "center", color: "rgba(255,255,255,0.2)", padding: 30 }}>Sin verificaciones aun.</div> :
          checks.map(c => (
...
function MeasurementsTab({ client }) {
  ...
  async function save() {
    ...
    setList(await db.getMeasurements(client.id));
    setOpen(false);
  }
  return (
    <div style={{ padding: 14 }}>
      <button onClick={() => setOpen(!open)} style={{ ...btnPrimary, width: "100%", marginBottom: 14 }}>+ Nueva Medicion</button>
      {open && (
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                <span style={{ color: "#fff", fontSize: 14, letterSpacing: 1 }}>Jose Leiro</span>
              ...
              <div style={{ color: "rgba(255,255,255,0.22)", fontSize: 9 }}>Panel de Gestion</div>
            </div>
          </div>
        </div>
        <div style={{ flex: 1, overflowY: "auto" }}>
